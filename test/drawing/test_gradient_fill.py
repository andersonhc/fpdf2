...
def test_linear_gradient_objbox_scale_translate(tmp_path: Path):
    ...
    lg = LinearGradient(0, 0, 1, 1, colors=["#0000FF", "#FFFFFF", "#FF0000"])
    ...
    assert_pdf_equal(pdf, HERE / "generated_pdf" / "gradient_linear_scale_translate.pdf", tmp_path, generate=True)
...
def test_linear_gradient_userspace_custom_pivot(tmp_path: Path):
    ...
    assert_pdf_equal(pdf, HERE / "generated_pdf" / "gradient_userspace_pivot.pdf", tmp_path, generate=True)
...
def test_linear_gradient_color_stops_extends(extend_before, extend_after, basename, tmp_path):
    ...
    p = _rect(30, 250, 260, 40)
    lg = LinearGradient(0, 0, 1, 0, colors=[
        "#0000FF",
        "#00FFFF",
        "#00FF00",
        "#FFFF00",
    ...
    paint = GradientPaint(lg, units="objectBoundingBox")
